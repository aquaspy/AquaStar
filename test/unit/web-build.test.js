...
test('web build reuses desktop pages through preload-compatible bridges', () => {
  ...
  ['reminders.html', 'todo.html', 'strategy.html', 'reminders_default.json', 'strategy_default.json'].forEach((name) =>
    assert.ok(build.indexOf(name) !== -1)
  );
  ['reminders.js', 'todo.js', 'strategy.js'].forEach((name) =>
    assert.ok(build.indexOf("bridges/' + " + 'f') !== -1 || build.indexOf(name) !== -1)
  );
  assert.ok(
    build.indexOf('Icon/Icon.png') !== -1 && build.indexOf('aquastar-icon.png') !== -1,
    'landing must include the AquaStar icon'
  );
  assert.ok(
