...
test('WikiView injection is retryable and supports hydrated Wiki content', () => {
  ...
  assert.ok(
    instances.indexOf('window.__aquastarWikiViewInjected = true; return true;') !== -1,
    'success is marked only after source execution'
  );
  ...
  assert.ok(
    hover.indexOf('link.startsWith("http://aqwwiki.wikidot.com/")') === -1,
    'hover no longer rejects HTTPS links'
  );
  ...
  assert.ok(
    instances.indexOf("'Wiki frame cleanup'") !== -1,
    'frame cleanup is explicitly scoped to the Wiki'
  );
  assert.ok(
