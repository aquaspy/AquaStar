...
test('Char Page Studio loads an explicitly requested Char Page into native Flash controls', () => {
    ...
    const captureProcess = fs.readFileSync(path.join(__dirname, '../../scripts/charpage-studio-capture-process.js'), 'utf8');
    assert.ok(captureProcess.indexOf('await delay(5000)') !== -1, 'high-resolution capture must wait for AQW assets');
    assert.ok(captureProcess.indexOf('movieRatio = 715 / 455') !== -1, 'capture must preserve the original Char Page proportion');
    assert.ok(captureProcess.indexOf('image.crop(pixelRect)') !== -1, 'capture must trim the fitted Flash area from the monitor-sized framebuffer');
    assert.ok(fs.readFileSync(path.join(__dirname, '../../res/features/charpage/lab/capture.html'), 'utf8').indexOf('application/x-shockwave-flash') !== -1, 'capture page must render native Flash');
