...
test('Char Page Studio loads an explicitly requested Char Page into native Flash controls', () => {
    ...
    ['player-name', 'load-character', 'initialFlashVars = \'\'', 'colorDefinitions', 'intColorHair', 'intColorSkin', 'intColorEye', 'intColorBase', 'intColorTrim', 'intColorAccessory', 'show-weapon', 'items/swords/unarmed.swf', 'strCustWeaponFile', 'bgindex', 'max="31"', 'characterBLab.loadCharacter', 'characterBLab.getDefaults', 'application/x-shockwave-flash', 'player.src = currentSourceUrl', 'diagnostics', 'onDiagnostic'].forEach((fragment) => {
    ...
    assert.ok(launcher.indexOf("charpage-lab-runtime-status") !== -1);
