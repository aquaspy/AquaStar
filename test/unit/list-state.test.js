...
test('list state reorders only inside the requested section', () => {
    const items = [
        { id: 'a', section: 'main' }, { id: 'b', section: 'seasonal' },
        { id: 'c', section: 'main' }, { id: 'd', section: 'seasonal' }
    ];
    const reordered = state.reorderInSection(items, 'c', 'a', (item) => item.section);
    assert.deepStrictEqual(reordered.map((item) => item.id), ['c', 'b', 'a', 'd']);
    assert.strictEqual(state.reorderInSection(items, 'c', 'b', (item) => item.section), items);
});
