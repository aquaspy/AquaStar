...
test('feature window controller creates, focuses and releases singleton windows', () => {
    class FakeWindow {
        constructor(config) { this.config = config; this.destroyed = false; this.handlers = {}; this.focused = 0; }
        isDestroyed() { return this.destroyed; }
        focus() { this.focused++; }
        setMenuBarVisibility(value) { this.menuVisible = value; }
        setTitle(value) { this.title = value; }
        loadURL(value) { this.url = value; }
        on(name, handler) { this.handlers[name] = handler; }
        close() { this.destroyed = true; this.handlers.closed(); }
    }
    const controller = createController({ settings: { config: { width: 1 }, title: 'Settings', url: 'file:///settings.html' } }, FakeWindow);
